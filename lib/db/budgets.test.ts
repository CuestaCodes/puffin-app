...
describe('Budget Operations', () => {
  ...
  describe('Initialize Monthly Budgets', () => {
    ...
    it('should be idempotent when run twice for the same month', () => {
      // The budget page fires an initialize on mount and on every month change, so the
      // same month gets initialized repeatedly. Running twice must not duplicate rows or
